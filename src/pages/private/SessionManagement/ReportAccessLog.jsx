...
import { reportAccessLog } from "../../../data/security";
...
const ReportAccessLog = () => {
  ...
  const filterAccessLog = reportAccessLog.filter(item => {
    return (
    ...
    <div className="box-container">
      ...
      <div className="container-head has-content-between flex-wrap">
        <div className="title-left">
          <PageTitle title="Report System Access Log" code="VRI-SC" />
        </div>
          ...
          <table className="table-list min-w-1199">
            <thead>
              <tr>
                ...
                <th className="text-center" style={{ width: "130px", maxWidth: "130px" }}>Log In</th>
                <th className="text-center" style={{ width: "130px", maxWidth: "130px" }}>Log Out</th>
            ...
            <tbody>
              { toggleData && filterAccessLog.length > 0 ? (
                filterAccessLog.map((employee) =>
                    ...
                    sessions.map((session, sessionIndex) => {
                      ...
                      return (
                        <tr key={`${employee.employeeId}-${date}-${session.session}`}>
                          ...
                          <td className={clsx("text-center", { "no-border-bottom": !isLastRowOfEmployee })}>{session.logIn}</td>
                          <td className={clsx("text-center", { "no-border-bottom": !isLastRowOfEmployee })}>{session.logOut}</td>
                          <td className={clsx("text-center", { "no-border-bottom": !isLastRowOfEmployee })}>{formatDuration(session.durationMinutes)}</td>
                        </tr>
                      )
                    })
                  ))
